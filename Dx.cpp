...
void ResizeD3D11(int width, int height)
{
    ...
    g_pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&pBack));
    ...
    depthDesc.Usage              = D3D11_USAGE_DEFAULT;
    depthDesc.BindFlags          = D3D11_BIND_DEPTH_STENCIL;
    g_pDevice->CreateTexture2D(&depthDesc, nullptr, &g_pDepthStencil);
    g_pDevice->CreateDepthStencilView(g_pDepthStencil, nullptr, &g_pDSV);
